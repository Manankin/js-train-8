...
function getUsersFromDatabase(array) {
  ...
  for (let item of usersArray) {
      function nameChange(item) {
        item.firstname = item.name.toUpperCase();
        delete item.name;
        item.years = item.age;
        delete item.age;
        return item;
...
function checkArray(obj) {
  ...
  if (obj === null || obj === undefined) {
    ...
    (Array.isArray(obj)) ? result = "true" : result = "false";
  }
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (Array.isArray(arr) && start >= 0 && start <= arr.length -1 && end <= arr.length-1) {
...
function customUnshift(arr, ...elements) {
  ...
  if (Array.isArray(arr)) {
    ...
    // let arrToAdd = elements;
    for (let index = elements.length - 1; index >= 0; index--) {
      arr.unshift(elements[index]);
    }
    let endLength = arr.length;
    return {initialLength: startLength, newLength: endLength, arr: arr};
  } else { return false }
